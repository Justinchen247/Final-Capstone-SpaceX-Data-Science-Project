...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fil = (filtered_df['Launch Site'] == entered_site) 
        filtered_df = filtered_df[fil]
        filtered_df.reset_index(inplace=True)
        filtered_df = filtered_df.groupby(['class'], as_index=False).count()
        fig = px.pie(filtered_df, values='index', 
        names='class', 
        title='Total success rate for the site {}'.format(entered_site))
        return fig
# TASK 4:
...
def get_scatter_plot(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        ...
        fig2 = px.scatter(spacex_df[mask], x='Payload Mass (kg)', y='class', color='Booster Version Category',
        ...
        filtered_df.reset_index(inplace=True)
        mask = (spacex_df['Payload Mass (kg)'] > low) & (spacex_df['Payload Mass (kg)'] < high)
        fig2 = px.scatter(filtered_df[mask], x='Payload Mass (kg)', y='class', color='Booster Version Category',
